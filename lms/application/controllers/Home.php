...
class Home extends CI_Controller {
    ...
    public function download($filename = "") {
        ...
        $yourFile = base_url().'uploads/'.$this->config->item('upload_directory').'/lesson_files/'.$filename;
    ...
    public function login() {
        ...
        $page_data['loginURL'] = $this->google->loginURL();
    ...
    public function submit_quiz($from = "") {
        ...
        $enroll_submit_quiz = $this->crud_model->enroll_submit_quiz($quiz_id);
        var_dump($_POST);

        $total_correct_answers = 0;
        foreach ($quiz_questions as $quiz_question) {
            if($quiz_question['type']=='multiple_choice' || $quiz_question['type']=='multiple_response' || $quiz_question['type']=='true_or_false'){
            ...
            }else if($quiz_question['type']=='short_answer' || $quiz_question['type']=='essay' || $quiz_question['type']=='numeric'){
                if($quiz_question['type']=='short_answer' && strlen($this->input->post('answer_'.$quiz_question['id']))>=5){
                    $total_correct_answers++;
                    $submitted_answer_status = 1;
                }else if($quiz_question['type']=='essay' && strlen($this->input->post('answer_'.$quiz_question['id']))>=100){
                    $total_correct_answers++;
                    $submitted_answer_status = 1;
                }else if($quiz_question['type']=='numeric' && strlen($this->input->post('answer_'.$quiz_question['id']))>=1){
                    $total_correct_answers++;
                    $submitted_answer_status = 1;
                }else{
                    $submitted_answer_status = 0;
                 ...
                 $container = array(
                    ...
                    "question_type" => $quiz_question['type'],
                    'submitted_answer_status' => $submitted_answer_status,
                    "submitted_answers" => $this->input->post('answer_'.$quiz_question['id']),
                    "correct_answers"  => '',
                );
                array_push($submitted_quiz_info, $container);
            }else if($quiz_question['type']=='drag_and_drop' || $quiz_question['type']=='sequence' || $quiz_question['type']=='matching' || $quiz_question['type']=='drag_the_words' ){
               
                if($quiz_question['correct_answers'] == json_encode($this->input->post('sequence_'.$quiz_question['id']))){
                    $total_correct_answers++;
                    $submitted_answer_status = 1;
                }else{
                    $submitted_answer_status = 0;
                 ...
                 $container = array(
                    ...
                    "question_type" => $quiz_question['type'],
                    'submitted_answer_status' => $submitted_answer_status,
                    "submitted_answers" => json_encode($this->input->post('sequence_'.$quiz_question['id'])),
                    "correct_answers"  => $quiz_question['correct_answers'],
                );
                array_push($submitted_quiz_info, $container);
            }else if($quiz_question['type']=='fill_in_the_blanks' || $quiz_question['type']=='select_from_lists'){
                $q_title_a = explode(' ', $quiz_question['title']);

                    
                    foreach ($this->input->post('fill_in_the_blanks_'.$quiz_question['id']) as $fnbk => $fill_in_the_blanks) {
                        if(strcasecmp(str_replace(' ','',$fill_in_the_blanks),str_replace(' ','',$q_title_a[$fnbk+1]))==0){
                            $submitted_answer_status = 1;
                        }else{
                            $submitted_answer_status = 0;
                        }
                ...
                if($submitted_answer_status==1){
                 ...
                 $container = array(
                    ...
                    "question_type" => $quiz_question['type'],
                    'submitted_answer_status' => $submitted_answer_status,
                    "submitted_answers" => json_encode($this->input->post('fill_in_the_blanks_'.$quiz_question['id'])),
                    "correct_answers"  => $quiz_question['title'],
                );
                array_push($submitted_quiz_info, $container);
            }
