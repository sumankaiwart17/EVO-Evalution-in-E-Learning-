...
class Mobile extends CI_Controller
{
    ...
    function get_image_url($type = '', $id = '')
    {
        ...
        $response = array();
        if (file_exists('uploads/'.$this->config->item('upload_directory').'/' . $type . '_image/' . $id . '.jpg'))
            $response['image_url'] = base_url() . 'uploads/'.$this->config->item('upload_directory').'/' . $type . '_image/' . $id . '.jpg';
        else
            $response['image_url'] = base_url() . 'uploads/'.$this->config->item('upload_directory').'/user.jpg';
        echo json_encode($response);
    }
    ...
    function update_user_image()
    {
        ...
        $directory = 'uploads/'.$this->config->item('upload_directory').'/' . $user_type . '_image/' . $user_id . '.jpg';
    ...
    function get_message_threads() {
        ...
        foreach ($threads as $row) {
            ...
            if (file_exists('uploads/'.$this->config->item('upload_directory').'/'.$user_type.'_image/'.$user_id.'.jpg'))
                $image_url = base_url('uploads/'.$this->config->item('upload_directory').'/'.$user_type.'_image/'.$user_id.'.jpg');
            else
                $image_url = base_url('uploads/'.$this->config->item('upload_directory').'/user.jpg');
