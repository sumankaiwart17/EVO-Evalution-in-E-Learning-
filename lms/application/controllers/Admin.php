...
class Admin extends CI_Controller {
    ...
    public function course_actions($param1 = "", $param2 = "") {
        echo $this->input->post('course_media_files');
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
    ...
    function manage_multiple_choices_options() {
        ...
        $page_data['option_type'] = $this->input->post('option_type');
        
        if($this->input->post('question_type')=='select_from_lists' || $this->input->post('question_type')=='drag_the_words' || $this->input->post('question_type')=='drag_and_drop' || $this->input->post('question_type')=='matching' || $this->input->post('question_type')=='sequence'){
            $page_data['type'] = 'number';
        }else{
            $page_data['type'] = 'checkbox';
        }
        if($this->input->post('option_type')=='image'){
            $this->load->view('backend/admin/manage_multiple_choices_options_image', $page_data);
        }else if($this->input->post('option_type')=='text'){
