...
<?php foreach ($submitted_quiz_info as $each):
    ...
    if($each['question_type']=='short_answer' || $each['question_type']=='essay' || $each['question_type']=='numeric' || $each['question_type']=='numeric'){
        $submitted_answers = $each['submitted_answers'];
    }else{
    ...
    <div class="col-lg-12">
        <div class="card text-left card-with-no-color-no-border">
            <div class="card-body">
                ...
                <?php if($question_details['type']=='multiple_choice' || $question_details['type']=='multiple_response' || $question_details['type']=='true_or_false' || $question_details['type']=='sequence' || $question_details['type']=='matching' || $question_details['type']=='drag_the_words' || $question_details['type']=='drag_and_drop'): ?>
                ...
                <?php endif; ?> 
                <?php if($question_details['type']=='short_answer' || $question_details['type']=='essay' || $question_details['type']=='numeric'): ?>
                    
                    <p class="card-text"> <strong><?php echo get_phrase("submitted_answers"); ?>: </strong> [
                        <?php
                            echo $submitted_answers;
                        ?>
                        ]</p>
                <?php endif; ?> 
                <?php if($question_details['type']=='fill_in_the_blanks' || $question_details['type']=='select_from_lists'): ?>
                    ...
                    <p class="card-text"> <strong><?php echo get_phrase("submitted_answers"); ?>: </strong> [
                       <?php foreach ($submitted_answers as $key3 => $submitted_answer): ?>
                            <?php
