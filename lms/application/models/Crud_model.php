...
class Crud_model extends CI_Model
{
    ...
    public function add_category()
    {
        ...
        if ($previous_data == 0) {
            if ($this->input->post('parent') == 0) {
                ...
                if (!file_exists('uploads/'.$this->config->item('upload_directory').'/thumbnails/category_thumbnails')) {
                    mkdir('uploads/'.$this->config->item('upload_directory').'/thumbnails/category_thumbnails', 0777, true);
                }
                if ($_FILES['category_thumbnail']['name'] == "") {
                    $data['thumbnail'] = 'category-thumbnail.png';
                } else {
                    $data['thumbnail'] = md5(rand(10000000, 20000000)) . '.jpg';
                    move_uploaded_file($_FILES['category_thumbnail']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/thumbnails/category_thumbnails/' . $data['thumbnail']);
                }
            }
    ...
    public function update_system_settings()
    {
        ...
        $data['value'] = html_escape($this->input->post('facebook_app_id'));
        $this->db->where('key', 'facebook_app_id');
        $this->db->update('settings', $data);

        $data['value'] = html_escape($this->input->post('facebook_app_secret'));
        $this->db->where('key', 'facebook_app_secret');
        $this->db->update('settings', $data);

        $data['value'] = html_escape($this->input->post('google_client_id'));
        $this->db->where('key', 'google_client_id');
        $this->db->update('settings', $data);

        $data['value'] = html_escape($this->input->post('google_client_secret'));
        $this->db->where('key', 'google_client_secret');
        $this->db->update('settings', $data);

        $data['value'] = html_escape($this->input->post('google_redirect_uri'));
        $this->db->where('key', 'google_redirect_uri');
        $this->db->update('settings', $data);
    ...
    public function add_course($param1 = "")
    {
        ...
        if (!file_exists('uploads/'.$this->config->item('upload_directory').'/thumbnails/course_thumbnails')) {
            mkdir('uploads/'.$this->config->item('upload_directory').'/thumbnails/course_thumbnails', 0777, true);
        }
        ...
        foreach ($course_media_files as $course_media => $size) {
            if ($_FILES[$course_media]['name'] != "") {
                move_uploaded_file($_FILES[$course_media]['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/thumbnails/course_thumbnails/' . $course_media . '_' . get_frontend_settings('theme') . '_' . $course_id . '.jpg');
            }
        }
    ...
    public function get_course_thumbnail_url($course_id, $type = 'course_thumbnail')
    {
        ...
        // if (file_exists('uploads/'.$this->config->item('upload_directory').'/thumbnails/course_thumbnails/'.$type.'_'.get_frontend_settings('theme').'_'.$course_id.'.jpg')){
        //     return base_url().'uploads/'.$this->config->item('upload_directory').'/thumbnails/course_thumbnails/'.$type.'_'.get_frontend_settings('theme').'_'.$course_id.'.jpg';
        // } elseif(file_exists('uploads/'.$this->config->item('upload_directory').'/thumbnails/course_thumbnails/'.$course_id.'.jpg')){
        //     return base_url().'uploads/'.$this->config->item('upload_directory').'/thumbnails/course_thumbnails/'.$course_id.'.jpg';
        // } else{
    ...
    public function get_lesson_thumbnail_url($lesson_id)
    {
        ...
        if (file_exists('uploads/'.$this->config->item('upload_directory').'/thumbnails/lesson_thumbnails/' . $lesson_id . '.jpg'))
        return base_url() . 'uploads/'.$this->config->item('upload_directory').'/thumbnails/lesson_thumbnails/' . $lesson_id . '.jpg';
        else
        return base_url() . 'uploads/'.$this->config->item('upload_directory').'/thumbnails/thumbnail.png';
    }
    ...
    public function add_lesson()
    {
        ...
        if ($lesson_type == 'video') {
            ...
            $s3config = array(
                ...
                if (!file_exists('uploads/'.$this->config->item('upload_directory').'/lesson_files/videos')) {
                    mkdir('uploads/'.$this->config->item('upload_directory').'/lesson_files/videos', 0777, true);
                }
                $video_file_path = 'uploads/'.$this->config->item('upload_directory').'/lesson_files/videos/' . $uploadable_video_file;
                ...
                if ($attachment_type == 'iframe') {
                    ...
                    if ($_FILES['attachment']['name'] == "") {
                        ...
                        if (!file_exists('uploads/'.$this->config->item('upload_directory').'/lesson_files')) {
                            mkdir('uploads/'.$this->config->item('upload_directory').'/lesson_files', 0777, true);
                        }
                        move_uploaded_file($_FILES['attachment']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/lesson_files/' . $uploadable_file);
                    }
                }
            }
            ...
            if ($_FILES['thumbnail']['name'] != "") {
                if (!file_exists('uploads/'.$this->config->item('upload_directory').'/thumbnails/lesson_thumbnails')) {
                    mkdir('uploads/'.$this->config->item('upload_directory').'/thumbnails/lesson_thumbnails', 0777, true);
                }
                move_uploaded_file($_FILES['thumbnail']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/thumbnails/lesson_thumbnails/' . $inserted_id . '.jpg');
            }
        }
        ...
        public function edit_lesson($lesson_id)
        {
            ...
            if ($lesson_type == 'video') {
                ...
                if ($lesson_provider == 'youtube' || $lesson_provider == 'vimeo') {
                    ...
                    if ($_FILES['thumbnail']['name'] != "") {
                        ...
                        move_uploaded_file($_FILES['thumbnail']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/thumbnails/lesson_thumbnails/' . $lesson_id . '.jpg');
                    }
                } else {
                    $this->session->set_flashdata('error_message', get_phrase('invalid_lesson_provider'));
                    redirect(site_url(strtolower($this->session->userdata('role')) . '/course_form/course_edit/' . $data['course_id']), 'refresh');
                    ...
                    if ($attachment_type == 'iframe') {
                        ...
                        if ($_FILES['attachment']['name'] != "") {
                            ...
                            if ($previous_data['attachment'] != "") {
                                unlink('uploads/'.$this->config->item('upload_directory').'/lesson_files/' . $previous_data['attachment']);
                            }
            ...
            {
                move_uploaded_file($_FILES['banner_image']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/system/home-banner.jpg');
            }

            public function update_light_logo()
            {
                move_uploaded_file($_FILES['light_logo']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/'.$this->config->item('upload_directory').'/system/logo-light.png');
            }

            public function update_dark_logo()
            {
                move_uploaded_file($_FILES['dark_logo']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/'.$this->config->item('upload_directory').'/system/logo-dark.png');
            }

            public function update_small_logo()
            {
                move_uploaded_file($_FILES['small_logo']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/system/logo-light-sm.png');
            }

            public function update_favicon()
            {
                move_uploaded_file($_FILES['favicon']['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/system/favicon.png');
            }
                ...
                foreach ($purchased_courses as $purchased_course) {
                    ...
                    if (get_user_role('role_id', $course_details['user_id']) == 1) {
                        ...
                        if (get_settings('allow_instructor') == 1) {
                            ...
                            $data['instructor_revenue'] = number_format((($data['amount'] * $instructor_revenue_percentage) / 100), 2, '.', '');
            ...
            function curl_request($code = '')
            {
                ...
                function add_multiple_choice_question($quiz_id)
                {
                    ...
                    $data['fill_in_the_blanks']            = json_encode($this->input->post('fill_in_the_blanks'));
                    $data['correct_answers']    = json_encode($correct_answers);
                    $data['answer']             = $this->input->post('answer');
                    $data['numeric_answer']     = $this->input->post('numeric_answer');

                    $this->db->insert('question', $data);

                     // Create folder if does not exist
                    if (!file_exists('uploads/'.$this->config->item('upload_directory').'/options/image')) {
                        mkdir('uploads/'.$this->config->item('upload_directory').'/options/image', 0777, true);
                    }

                    // Upload different number of images according to activated theme. Data is taking from the config.json file
                    $course_media_files = themeConfiguration(get_frontend_settings('theme'), 'option_1');
                    foreach ($course_media_files as $course_media => $size) {
                        if ($_FILES[$course_media]['name'] != "") {
                            move_uploaded_file($_FILES[$course_media]['tmp_name'], 'uploads/'.$this->config->item('upload_directory').'/options/image/' . $course_media . '_option_1 _' . get_frontend_settings('theme') . '_' . $course_id . '.jpg');
                        }
                    }
                    // Create folder if does not exist
                    // if (!file_exists('uploads/'.$this->config->item('upload_directory').'/thumbnails/course_thumbnails')) {
                ...
                function get_application_details()
                {
                    ...
                    public function submit_quiz($instructor_id) {
                        $course_ids = $this->crud_model->get_instructor_wise_courses($instructor_id, 'simple_array');
                        if (!count($course_ids) > 0) {
                            return false;
                        }
                    ...
                    public function enroll_submit_quiz($instructor_id) {
                        $course_ids = $this->crud_model->get_instructor_wise_courses($instructor_id, 'simple_array');
                        if (!count($course_ids) > 0) {
                            return false;
                        }
